...
`packages/core` に Unit テストを実装し、CI（GitHub Actions）で毎 push 実行しています。方針の詳細は [docs/testing.md](docs/testing.md)。

- **現状の実装**: コアロジック（重み付き出題アルゴリズム、Leitner Box習熟度判定）の Unit テスト
- **今後の目標**: 
  - `QuizViewModel` をはじめとする主要 ViewModel の状態遷移テストの拡充
  - `WidgetTester` を用いた主要 UI コンポーネントのテスト
  - `integration_test` を用いた E2E テストの導入
- **モック**: 手書き Fake Repository を基本とし、呼び出し検証が要る箇所のみ `mocktail` を使用
...
- **ViewModel のテスト**: これまで未カバーでしたが、現在 `QuizViewModel` の状態遷移テスト（Riverpodの `ProviderContainer` と `mocktail` を活用）の導入を開始しています。
