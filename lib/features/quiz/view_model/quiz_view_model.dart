...
class QuizViewModel extends _$QuizViewModel {
  ...
  static const _allThemes = {
    'frequent', 'nature', 'daily', 'people', 'place',
    'action', 'mind', 'quality', 'school', 'general',
  };

  /// テーマフィルターをトグルする。
  /// 空集合 = すべて選択状態。空のときタップ → そのテーマだけ除外して残りを選択。
  /// 最後の1つを外す → 空（全選択）に戻る。
  Future<void> toggleThemeFilter(String theme) async {
    final Set<String> next;
    if (state.selectedThemes.isEmpty) {
      next = Set<String>.from(_allThemes)..remove(theme);
    } else if (state.selectedThemes.contains(theme)) {
      final removed = Set<String>.from(state.selectedThemes)..remove(theme);
      next = removed;
    } else {
      next = Set<String>.from(state.selectedThemes)..add(theme);
    }
    state = state.copyWith(selectedThemes: next);
    await _updateFilteredWordCount();
  }
  ...
  String _levelToString(LevelFilter level) => switch (level) {
        ...
        LevelFilter.eikenPre2 => 'eiken_pre2',
        LevelFilter.eiken2 => 'eiken2',
        LevelFilter.toiecBasic => 'toeic_basic',
  ...
  /// SRS 3スロット方式で [count] 件の問題を選出・構築する。
  Future<List<QuizQuestion>> _pickQuestions(int count) async {
    if (_allWords.isEmpty) {
      return [];
    }
    ...
    return picked
        .map((e) => _buildQuestion(e.word, e.record?.stage ?? 0))
        .toList();
  }

  /// 1問分の [QuizQuestion] を構築する。ステージから問題形式を決定する。
  QuizQuestion _buildQuestion(QuizWord word, int stage) {
    final format = _formatForStage(stage);

    // letterTap 形式: 各文字スロットの選択肢を生成して返す
    ...
    if (!isTyping) {
      final sameThemeAndPos = _allWords
          .where(
            (w) =>
                w.id != word.id &&
                w.theme == word.theme &&
      ...
      final others = _allWords
          .where((w) => w.id != word.id && w.theme != word.theme)
          .toList()
        ..shuffle(_rng);
      final decoyCandidates = [...sameThemeAndPos, ...sameTheme, ...others];
