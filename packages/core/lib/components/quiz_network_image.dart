...
/// タイムアウト付きキャッシュマネージャ。
///
/// デフォルトの [DefaultCacheManager] はダウンロードタイムアウトを持たないため、
/// DNS 解決に失敗した場合でも [CachedNetworkImage] が loading 状態のまま永遠に
/// フリーズする。15 秒でタイムアウトさせることで errorWidget へ遷移させる。
class _QuizImageCacheManager extends CacheManager with ImageCacheManager {

  factory _QuizImageCacheManager() => _instance;

  _QuizImageCacheManager._()
    : super(Config(_key, fileService: _TimeoutHttpFileService()));
  static const _key = 'quizImageCache';

  static final _QuizImageCacheManager _instance = _QuizImageCacheManager._();
}
...
class _TimeoutHttpFileService extends FileService {
  ...
  Future<FileServiceResponse> get(String url, {Map<String, String>? headers}) =>
      _inner.get(url, headers: headers).timeout(_timeout);
}
              ...
              child: FittedBox(
                ...
                child: Hero(
                  ...
                  child: CachedNetworkImage(
                    imageUrl: url,
                    cacheManager: _QuizImageCacheManager(),
                    fit: BoxFit.none,
